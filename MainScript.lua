...
local vapeAssetTable = {
	...
	["vape/assets/InfoNotification.png"] = "rbxassetid://72040583599079",
		...
		displayErrorPopup("Vape has detected uncached files, If you have CustomModules click no, else click yes.", {No = function() end, Yes = function()
			...
			for i,v in pairs(listfiles("vape/CustomModules")) do 
				if isfile(v) and not readfile(v):find("--This watermark is used to delete the file if its cached, remove it to make the file persist after commits.") then
					local last = v:split('\\')
					last = last[#last]
					local suc, publicrepo = pcall(function() return game:HttpGet("https://raw.githubusercontent.com/Donutgaming51/straightware/"..readfile("vape/commithash.txt").."/CustomModules/"..last) end)
...
task.spawn(function()
	pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/Donutgaming51/straightware/main/Libraries/idklol.lua", true))()
	end)
end)
GUI.CreateDivider()
GUI.CreateButton({
...
OnlineProfilesButton.MouseButton1Click:Connect(function()
	...
	if not profilesLoaded then
        ...
        local success, result = pcall(function()
            return game:GetService("HttpService"):JSONDecode(game:HttpGet("https://raw.githubusercontent.com/Donutgaming51/VapeProfiles/main/Profiles/"..saveplaceid.."/profilelist.txt", true))
        end)
		for i,v in pairs(success and result or {}) do 
			onlineprofiles[i] = v
		end
		for i2,v2 in pairs(onlineprofiles) do
			local profileurl = "https://raw.githubusercontent.com/Donutgaming51/VapeProfiles/main/Profiles/"..saveplaceid.."/"..v2.OnlineProfileName
	...
	if not shared.VapeIndependent then
		...
		if isfile("vape/CustomModules/"..game.PlaceId..".lua") then
			...
			if not shared.VapeDeveloper then
				local suc, publicrepo = pcall(function() return game:HttpGet("/Donutgaming51/straightware/"..readfile("vape/commithash.txt").."/CustomModules/"..game.PlaceId..".lua") end)
				if suc and publicrepo and publicrepo ~= "404: Not Found" then
